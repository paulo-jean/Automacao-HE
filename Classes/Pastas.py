...
class Pastas:
    ...
    def criar_pasta(self, destino):
        ''' função para criar a pasta dos diarios da respectiva turma.

            Argumentos:
            self -> respectiva turma |
            destino -> caminho escolhido pelo usuario
        '''
        caminho_completo = os.path.join(destino, self._caminho)
        try:
    ...
    def mover_arquivos(self, pasta_downloads, destino):
        ...
        try:
            ...
            for arquivo in arquivos:
             ...
             messagebox.showerror(title='Erro 0x002',
                                  message=f'''Não foi possível mover os arquivos!
Um ou mais arquivos já existem na pasta de destino:
